...
for (file, commit), group in df.groupby(["Changed File", "Commit ID"]):
    ...
    if baseline.empty:
        # TODO: dont want to skip this. just means there are no changes. need to handle this case.
        continue
...
# box for Misses
plt.subplot(1, 2, 1)
plt.boxplot(data_misses, labels=approaches)
plt.title("Misses Distribution by Approach")
plt.ylabel("Misses")

# b for Hallucinations.
plt.subplot(1, 2, 2)
plt.boxplot(data_halluc, labels=approaches)
...
for approach in sorted(diff_df["Approach"].unique()):
    ...
    sns.lineplot(x=percentiles, y=perc_values, marker="o", label=approach)
